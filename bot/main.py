...
def main():
    ...
    telegram_config = {
        ...
        'token_price': float(os.environ.get('TOKEN_PRICE', 0.006)),
        'image_prices': [float(i) for i in os.environ.get('IMAGE_PRICES', "0.032,0.036,0.04").split(",")],
        'vision_token_price': float(os.environ.get('VISION_TOKEN_PRICE', '0.01')),
        'image_receive_mode': os.environ.get('IMAGE_FORMAT', "photo"),
        'tts_model': os.environ.get('TTS_MODEL', 'tts-1'),
        'tts_prices': [float(i) for i in os.environ.get('TTS_PRICES', "0.03,0.060").split(",")],
